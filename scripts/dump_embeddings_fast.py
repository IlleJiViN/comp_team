...
# Use named cursor for server-side fetching, NO ORDER BY!
named_cur = conn.cursor('server_side_cursor')
named_cur.itersize = batch_size
named_cur.execute("SELECT id, embedding_vector_v4 FROM places WHERE embedding_vector_v4 IS NOT NULL")
...
sort_idx = np.argsort(final_ids)
final_ids = final_ids[sort_idx]
final_vectors = final_vectors[sort_idx]

...
# Use named cursor for server-side fetching
named_cur = conn.cursor('server_side_cursor')
named_cur.itersize = batch_size
named_cur.execute("SELECT id, embedding_vector_v4 FROM places WHERE embedding_vector_v4 IS NOT NULL ORDER BY id")
